 ...
 * Paste this directly into the Cloudflare Dashboard editor (worker.js)
...
// Turso exposes a REST API at /v2/pipeline — we use it directly with fetch.
function getDb(env) {
  // Turso URL must be https:// not libsql://
...
function getRedis(env) {
  ...
  return {
    ...
    async lpush(key, value) {
      await fetch(`${url}/lpush/${encodeURIComponent(key)}`, { method: 'POST', headers, body: JSON.stringify([value]) });
    },
  ...
  if (formConfig.slack_enabled && formConfig.slack_bot_token && formConfig.slack_channel_id) {
    tasks.push((async () => {
      try {
        ...
        const fieldsResult = await db.execute({ sql: 'SELECT id, label FROM form_fields WHERE form_id = ?', args: [formConfig.id] });
        const blocks = [
        ...
        if (res.ok) await db.execute({ sql: 'UPDATE submissions SET slack_synced = 1 WHERE id = ?', args: [submissionId] });
  ...
  if (formConfig.zapier_enabled && formConfig.zapier_webhook_url) {
    tasks.push((async () => {
      try {
        ...
        if (res.ok) await db.execute({ sql: 'UPDATE submissions SET zapier_synced = 1 WHERE id = ?', args: [submissionId] });
  ...
  if (formConfig.airtable_enabled && formConfig.airtable_api_key && formConfig.airtable_base_id) {
    tasks.push((async () => {
      try {
        ...
        if (res.ok) await db.execute({ sql: 'UPDATE submissions SET airtable_synced = 1 WHERE id = ?', args: [submissionId] });
    ...
    tasks.push((async () => {
      try {
        ...
        if (accessToken) {
          ...
          if (res.ok) await db.execute({ sql: 'UPDATE submissions SET google_synced = 1 WHERE id = ?', args: [submissionId] });
        }
    ...
    tasks.push((async () => {
      try {
        ...
        await fetch('https://api.notion.com/v1/pages', {
          ...
          body: JSON.stringify({ parent: { database_id: formConfig.notion_database_id }, properties }),
        });
...
async function getGoogleAccessToken(env, db, userId) {
  try {
    ...
    const res = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({ client_id: env.GOOGLE_CLIENT_ID, client_secret: env.GOOGLE_CLIENT_SECRET, refresh_token: integration.refresh_token, grant_type: 'refresh_token' }),
    });
    const d = await res.json();
    if (!res.ok) return null;
    await db.execute({
      sql: 'UPDATE user_integrations SET access_token = ?, expires_at = ? WHERE id = ?',
      args: [d.access_token, new Date(Date.now() + d.expires_in * 1000).toISOString(), integration.id],
    });
...
async function processQueue(env) {
  ...
  try {
    while (Date.now() - startTime < MAX_RUNTIME_MS) {
      ...
      for (let i = 0; i < BATCH_SIZE; i++) {
        ...
        try {
          ...
          if (status === 'completed') { console.log(`[Worker] Skip completed msg:${item.msg_id}`); continue; }
          batch.push(item);
        } catch {
      ...
      if (!batch.length) { console.log(`[Worker ${workerId}] Queue empty.`); break; }
      console.log(`[Worker ${workerId}] Processing ${batch.length} items`);
      ...
      for (const item of batch) {
        ...
        if (env.TURNSTILE_SECRET_KEY && env.TURNSTILE_SECRET_KEY !== 'your_secret_key_here' && item.captchaToken) {
          try {
            ...
            if (!vData.success) { await pushDeadLetter(redis, item, `Turnstile rejected`); continue; }
          } catch (e) { console.warn('[Turnstile Error]', e.message); }
        }
        verified.push(item);
      }

      if (!verified.length) { await redis.del(processingList); continue; }
      ...
      for (const item of verified) {
        try {
          ...
          await db.execute({
            sql: 'INSERT INTO submissions (id, form_id, data, submitted_at) VALUES (?, ?, ?, ?)',
            args: [subId, item.form_id, dataJson, submittedAt],
          });
          if (Array.isArray(item.files) && item.files.length > 0) {
            for (const f of item.files) {
              await db.execute({
                sql: 'INSERT INTO files (id, submission_id, file_path, file_name, file_size, mime_type) VALUES (?, ?, ?, ?, ?, ?)',
                args: [crypto.randomUUID(), subId, f.path || f.url || '', f.fileName || f.name || 'unknown', f.size || 0, f.mimeType || f.mime_type || 'application/octet-stream'],
              }).catch(e => console.error('[File Insert Error]', e.message));
            }
          }
      ...
      for (const fid of uniqueFormIds) {
        try {
          ...
          if (cached) { configMap.set(fid, typeof cached === 'string' ? JSON.parse(cached) : cached); continue; }
          const result = await db.execute({ sql: 'SELECT * FROM forms WHERE id = ?', args: [fid] });
          if (result.rows.length) {
            configMap.set(fid, result.rows[0]);
            await redis.setex(`form:${fid}:meta`, 60, JSON.stringify(result.rows[0])).catch(() => {});
          }
        } catch (e) { console.error(`[Config Load Error] form:${fid}`, e.message); }
      }

      // Run integrations + mark complete
      await Promise.allSettled(verified.map(async (item) => {
        ...
        if (formConfig) await runIntegrations(env, db, formConfig, subId, item.data ?? {}).catch(e => console.error('[Integration Error]', e.message));
    ...
    console.log(`[Worker ${workerId}] Done. Processed: ${totalProcessed}`);
    return { success: true, processed: totalProcessed };
  } catch (err) {
    console.error(`[Worker ${workerId}] Fatal:`, err.message);
    await redis.del(processingList).catch(() => {});
